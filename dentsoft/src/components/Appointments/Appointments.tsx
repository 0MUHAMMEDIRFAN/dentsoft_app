...
function Appointments() {
    ...
    } = getPatientAppointmentList(undefined, selectedPatient.name)
                                        ...
                                        <tr key={index} className='box-border h-[60px]'>
                                            ...
                                            <td>
                                                <div className='flex items-center gap-4'>
                                                    <p className={`${data?.status === "Completed" ? "text-[#5ABA53]" : data?.Status === "Declined" ? "text-[#FF2727]" : "text-[#FE9F2F]"} overflow-hidden flex items-center`}>
                                                        {data?.status}
                                                    </p>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div className='flex items-center gap-2'>
                                                    ...
                                                    <p className={`${data?.status === "Completed" ? "text-[#5ABA53]" : data?.status === "Declined" ? "text-[#FF2727]" : "text-[#FE9F2F]"} overflow-hidden whitespace-nowrap text-ellipsis flex items-center`}>
                                                        {data?.status}
                                                    </p>
                                                </div>
                                            </td>
                    ...
                    {!appointmentsLoading && (
                        appointmentsError ?
                            <div className='flex flex-col justify-center items-center'>
                                ...
                                <button 
                                    className='bg-[#4285F4] rounded h-[20px] px-2 font-medium text-white hover:bg-[#2070F5]' 
